// RUN: %{python_executable} %{imex_runner} -i %s --pass-pipeline-file=ptensor.pp -e main -entry-point-result=void --shared-libs=%{mlir_c_runner_utils} --shared-libs=%{mlir_runner_utils} | FileCheck %s
